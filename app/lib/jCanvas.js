    ...
    v.stage = (function () {
        ...
        var draw = {
            ...
            circle: function (ctx, prop) {
                ctx.beginPath();
                ctx.arc(prop.cx(), prop.cy(), prop.r(), prop.sAngle(), prop.eAngle(), prop.counterclockwise());
            ...
            text: function (ctx, prop) {
                ...
                if (typeof fsize === "number") fsize += config.sizeUnit;
                s += fsize;


                var lh = prop.lineHeight();
                if (lh) {
                    if (typeof lh === "number") lh += config.sizeUnit;
                    s += "/" + lh;
                }
                ...
                if (_strokeJ(ctx, prop)) {
                    if (prop.width()) ctx.strokeText(prop.text(), prop.x(), prop.y(), prop.width());
                    else ctx.strokeText(prop.text(), prop.x(), prop.y());
                }
                if (_fillJ(ctx, prop)) {
                    if (prop.width()) ctx.fillText(prop.text(), prop.x(), prop.y(), prop.width());
                    else ctx.fillText(prop.text(),prop.x(), prop.y());
                }
            }
        };
        //开放绘制函数，方便扩展
